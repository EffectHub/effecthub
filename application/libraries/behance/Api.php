...
class Be_Api {
  ...
  public function createUserWip( $image_path, $title, array $tags, $description = '', $assoc = false ) {
	...
	$query_params[ 'scope' ]   = implode( '|', $scope );
  ...
  protected function _executeRequest( $method, $url, $request_body = false, $curl_params = array() ) {
    ...
    //$curl_params = $this->array_replace_recursive( $default_curl_params, $curl_params );
    foreach($curl_params as $key => $value) {
	    $default_curl_params[$key] = $value;
	}
	$curl_params = $default_curl_params;
