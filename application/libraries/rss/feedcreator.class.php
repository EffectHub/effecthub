...
class FeedCreator extends HtmlDescribable {
	...
	*/
	...
	function useCached($filename="", $timeout=3600) {
		...
		if ($filename=="") {
			$filename = base_url().'rss/feed.xml';
		}
	...
	function createFeed() { 
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			if ($this->items[$i]->date!=null) {
				...
				$feed.= "        <dc:date>".htmlspecialchars(strtotime($this->items[$i]->date))."</dc:date>\n";
			}
...
class PIECreator01 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "        <link>".htmlspecialchars($this->items[$i]->link)."</link>\n";
			//$itemDate = new FeedDate($this->items[$i]->date);
			$feed.= "        <created>".htmlspecialchars(strtotime($this->items[$i]->date))."</created>\n";
			//$feed.= "        <created>".htmlspecialchars($itemDate->iso8601())."</created>\n";
			$feed.= "        <issued>".htmlspecialchars(strtotime($this->items[$i]->date))."</issued>\n";
			$feed.= "        <modified>".htmlspecialchars(strtotime($this->items[$i]->date))."</modified>\n";
