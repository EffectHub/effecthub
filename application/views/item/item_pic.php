...
<script language="javascript" type="text/javascript" >
       ...
       $("#edit_html_addr").on("click",function(){$(this).select()});
		...
		<?php if ($item['from']=='particle'||$item['from']=='sea3d') {?> 
...
<img alt="<?php echo $item['author_name']; ?>" class="photo" src="<?php echo $item['author_pic']; ?>"></div></a>
			...
			<?php if ($list==null||$list==0||$tool==null){  ?>
			<a href="<?=site_url('item/featured/MostRecent/'.$item['type_link'])?>">
			<?php echo $lang==2?$item['type_name_cn']:$item['type_name']; ?></a>
			<?php  }else{?>
				<a href="<?php echo $item['type']==12?site_url('item/showcase/'.$tool['domain']):site_url('item/demo/'.$tool['domain']); ?>">
			<?php echo $tool['name']; ?> <?php echo $item['type']==12?'Showcase':'Demo'; ?></a>
				<?php  }?>
                ...
                <?php }else if($item['from']=='htmleditor'||$item['from']=='aseditor'){ ?>
                 ...
                 <?php  }else{?>
						...
						<?php if($item['download_url']!=0||$item['download_url']!=null){ ?>
							<?php if ($this->session->userdata('id')){  ?>
								<?php if(!$download){ ?>
									<a target="_blank" onclick="javascript: return cost();" href="<?=site_url('item/download/'.$item['id'])?>" style="width:120px">
		                       <strong class="title"><?= $this->lang->line('item_download'); ?></strong>
		                        </a>
		                        <?php  }else{?>
		                        <a target="_blank" href="<?=site_url('item/download/'.$item['id'])?>" style="width:120px">
		                       <strong class="title"><?= $this->lang->line('item_already_download'); ?></strong>
		                        </a>	
								<?php  } ?>	
		                         <?php  }else{?>
	...
	<div class="the-shot" style="width:100%">
		...
		<?php if(($item['pic_url']!=null&&$item['pic_url']!='')||($item['extension']=='plist')||($item['extension']=='swf')||($item['type']==8&&$item['extension']!='zip')||($item['type']==2&&$item['extension']=='zip')||$item['from']=='htmleditor'||($item['preview_url']!='0'&&$item['preview_url']!=null)||$item['from']=='particle'||$item['from']=='dragonbones'||$item['from']=='sea3d'){ ?>
<div class="single" style="width:660px;padding:10px;position:relative;">

<?php if($item['pic_url']!=null&&$item['pic_url']!=''&&($item['type']==3||$item['type']==4||$item['type']==7||($item['type']==12))){ ?>
<div style="width: 100%; height:auto;padding:0px;margin:0px;overflow-y: auto;">
<a title="<?php echo $item['type']==12?$this->lang->line('item_play_game'):$this->lang->line('item_next'); ?>" href="<?php echo $item['type']==12?site_url('item/fullscreen/'.$item['id']):site_url('item/'.($item['id']+1))?>"><img width="100%" alt="<?=$item['title']?>" src="<?=$item['pic_url']?>"></a>
</div>

<?php  }else{?>
<iframe id="item-preview" width="100%" height="600px" src="<?=site_url('item/embed/'.$item['id'])?>" scrolling="no"   frameborder="NO" border="0" framespacing="0"></iframe>
<?php  }?>
<?php  }else{?>
...
<?php  }?>
	...
	<?php if(($item['type']==2&&$item['extension']=='zip')||$item['from']=='htmleditor'||($item['preview_url']!='0'&&$item['preview_url']!=null)||$item['from']=='dragonbones'||$item['from']=='sea3d'){ ?>
                       <li><a target="_blank" href="<?=site_url('item/fullscreen/'.$item['id'])?>" class='buttonstyle'><?= $item['type']==12?$this->lang->line('item_play_game'):$this->lang->line('item_fullscreen'); ?></a></li>
                       <?php  }?>
                ...
                <?php }else if($item['from']=='particle2dx'){ ?>
                	<?php if ($this->session->userdata('id')&&$this->session->userdata('id')==$item['author_id']){?>
                ...
                <?php  }?>
                       ...
                       <?php if($item['download_url']!=0||$item['download_url']!=null){ ?>
                       	<?php if ($this->session->userdata('id')){  ?>
                       		<?php if(!$download){ ?>
                       			<li><a target="_blank" onclick="javascript: return cost();" href="<?=site_url('item/download/'.$item['id'])?>"  class='buttonstyle'><?= $this->lang->line('item_download'); ?>
                       		...
                       		<?php  }else{?>
                       			<li><a target="_blank" href="<?=site_url('item/download/'.$item['id'])?>"  class='buttonstyle'><?= $this->lang->line('item_already_download'); ?>
		                       	</a></li>
                       		<?php  } ?> 	
...
<?php  }?>
 ...
 <?php  }?>
                   ...
                   <?php if ($this->session->userdata('id')&&($this->session->userdata('id')==$item['author_id'])){  ?>
                   	...
                   	<?php }else if($item['from']=='particle2dx'){ ?>
                   			<a href="<?=site_url('particle2dx?id='.$item['id'])?>" class='buttonstyle'><?= $this->lang->line('item_edit_online'); ?></a>
                   			<a href="<?=site_url('item/edit/'.$item['id'])?>" class='buttonstyle'><?= $this->lang->line('item_edit_info'); ?></a>
                   	<?php  }else{?>
                   			<a href="<?=site_url('item/edit/'.$item['id'])?>" class='buttonstyle'><?= $this->lang->line('item_edit_info'); ?></a>
                   	<?php  } ?>
	...
	<div class="screenshot-meta item-info" style="float:left;width:680px">
		<div class="intro">
			...
			<div class="process" style="width:400px">
...
<?php }?>
	...
	<div id="comments-section" style="width:670px">
	...
	<?php if ($item_download_list != null) {?>
	<div>
	<h3><?= $this->lang->line('item_recent_downloads'); ?><span class="meta"></span></h3>
 	<ol class="prevnext group" style="margin-top:10px;margin-bottom:20px">
 		<div class="fans">
		<?php foreach($item_download_list as $_user): ?>
			<a href="<?php echo site_url('user/'.$_user['user_id']).'/'?>" style="margin:1px"><img height="30px" width="30px" title="<?php echo $_user['user_name']; ?>" src="<?php echo $_user['user_pic']; ?>" alt="<?php echo $_user['user_name']; ?>"></a>
		<?php endforeach; ?>
 		</div>
 	</ol>
...
<h3 class="more-from more-from-player"><?= $this->lang->line('item_information'); ?></h3>
  				...
  				<?php if ($item['price']!=null){  ?>
				                        ...
				                        $ <?php echo $item['price'].' '?>
				                        	<?php  } ?>
				                        <?php  } ?>&nbsp;&nbsp;
				                        <?php echo $item['download_num']; ?> <?= $this->lang->line('item_downloads'); ?>&nbsp;&nbsp;
  				...
  				<div style="display:block;margin-left:0px;font-size:12px;color:#aaa">
					...
					<span>
						<?php foreach($tags as $tag): ?>
								<a href="<?=site_url('item/tagSearch?tag='.$tag)?>" rel="tag" class="tag"><strong><?php echo $tag; ?></strong></a>
						<?php endforeach; ?>
					</span>
					</p>
				</div><br/>
...
<ul class="tools group" style="visibility: visible;">
	          <li class="fav">
	            <a><?php echo $item['fav_num']; ?></a>
	            </li>
	          <li class="cmnt">
	           <a><?php echo $item['comment_num']; ?></a>
	            </li>
	          <li class="views" style="margin-bottom:5px"><a><?php echo $item['view_num']; ?></a></li>
  				</ul></div><br/><br/>
<?php if ($item['tool']>0){  ?>
				<h3 class="more-from more-from-player"><?= $this->lang->line('item_created_using'); ?>
				</h3>
				<div style="display:block;clear:both;height:60px">
				<a target="_blank" title="<?php echo $tool['name']; ?>" href="<?=site_url('t/'.$tool['domain'])?>" style="float:left">
					<img width="60px" height="60px" style="border-radius:5px" alt="<?php echo $tool['name']; ?>" src="<?php echo $tool['thumb_url']; ?>">
  				</a>
  				<div style="float:left;margin-left:10px">
  				<a target="_blank" title="<?php echo $tool['name']; ?>" href="<?=$item['type']==12?site_url('item/showcase/'.$tool['domain']):site_url('item/demo/'.$tool['domain'])?>"><?php echo $tool['name']; ?></a>
  				<br/><br/>
  				<ul class="tools group" style="visibility: visible;">
	          <li>
	            <a target="_blank" href="<?=site_url('group/'.$tool['group_id'])?>"><?php echo $group['member_num']; ?> <?= $this->lang->line('tool_member'); ?></a>
	            </li>
	          <li style="margin-left:0">
	            <a target="_blank" href="<?=site_url('group/'.$tool['group_id'])?>"><?php echo $group['topic_num']; ?> <?= $this->lang->line('tool_topic'); ?></a>
	            </li>
	          	</ul>
  				</div>
  				</div>
...
<?php  }?>
	...
	<ol class="prevnext group" style="margin-top:10px">
	<?php foreach($tool_item_list as $_hot_item): ?>
                        <li>
		<a title="<?php echo $_hot_item['title']; ?>" href="<?=site_url('item/'.$_hot_item['id']).'?tool='.$_hot_item['tool']?>">
