...
<div id="main">
	...
	<div class="group-main">
		<div class="group-header">
			<h3><?= $this->lang->line('taskmine_all_responses'); ?></h3>
		</div>
        
        <div class="hot-topic">
        	<?php foreach($task_list as $_task): ?>
        	<div class="topic-item" style="height:auto">
        		<a href="<?= site_url('task/'.$_task['task']['id']) ?>" title="<?= $this->lang->line('taskexplore_view_comments'); ?>"><span class="t-comment-num"><?= $_task['task']['response_num'] >99?'99+':$_task['task']['response_num'] ?></span></a>
        		<span class="t-link" style="position:relative;left:5px">
        			<a href="<?= site_url('task/'.$_task['task']['id']) ?>" style="text-decoration:<?php echo $_task['task']['status']==1?'line-through':'';?>" title="<?=$_task['task']['title'] ?>">
        			[<?=$_task['task']['price']?>
				<?php if($_task['task']['price_type']=='1'){
					if($lang==2){
						echo '金币';
					}else{
						 echo 'coins';
        		...
        		<div class="parent-comment" style="margin:25px 0 0 50px">
        		<?php echo msubstr(strip_tags($_task['comment_content']),0,140); ?>
        		</div>
        		<span class="t-reply-time"><?= tranTime(strtotime($_task['update_date'])) ?></span>
