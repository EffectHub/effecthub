                   ...
                   <div class="mailstyle" name="<?php echo $this->session->userdata('id')?>">
                      ...
                      <div class="mailmain" style="margin-top:30px">
                         <div class="tabs">
                             ...
                             <a href="<?=site_url('user/checksendmail/'.$this->session->userdata('id'))?>" class="inboxstyle">Outbox</a>
 ...
 <?php foreach($mail_list as $_mail): ?>
  ...
  <div class="title">
    <div class="sender">
      ...
      <span class="from"><a href="<?=site_url('user/readmail/'.$_mail['id'].'/receiver_id')?>" style="color:#A1A1A1"><?php echo $_mail['author_name']; ?></a></span>
    </div>
    <div class="sender">
       <span class="time">  
         <!--<a rel="direct" title="Move to Junk Mail?" class="operate_link" href="<?=site_url('user/movesendmail/'.$_mail['id'])?>">Move to Junk Mail</a>-->
         <a title="Are you sure to delete this mail?" class="operate_link" href="<?=site_url('user/deletesendmail/'.$_mail['id'])?>" >Delete</a>
         <input type="checkbox" name="choosemail[]" value="<?php echo $_mail['id']; ?>">
       </span>
       <span class="from"><a href="<?=site_url('user/readmail/'.$_mail['id'].'/receiver_id')?>" class="url"><?php echo $_mail['content']; ?></a></span>
    </div>
  </div> 
  </li>
  <?php endforeach; ?>
...
<div class='paragraphstyle' style='font-weight: bold;font-size:16px;padding:10px;margin-bottom:20px'><?php echo $this->pagination->create_links();?></div>
                          ...
                          <div class="mailoperate"> 
                             <input type="submit" name="deletechosedmail" onclick="return confirm('Are you sure to delete choosed mail?')" value="Delete">
                              <!--<input type="submit" name="movechosedmail" value="Move to Junk Mail">-->
