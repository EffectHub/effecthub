	...
	<form method="post" action="<?=site_url('user/operatechosedmail/0')?>">
                   <div class="mailstyle" name="<?php echo $this->session->userdata('id')?>">
                      ...
                      <div class="mailmain" style="margin-top:30px">
                          <div class="tabs">
                             <a href="<?=site_url('user/checkmail/'.$this->session->userdata('id'))?>" class="inboxstyle">Inbox(<?php echo $unreadcount; ?> unread)</a>
                             <a href="<?=site_url('user/checkjunkmail/'.$this->session->userdata('id'))?>" class="junkmailstyle">Junk mail(<?php echo $unreadjunkcount; ?> unread)</a>
                             <a href="<?=site_url('user/checksendmail/'.$this->session->userdata('id'))?>" class="junkmailstyle">Outbox</a>
                             <div style="float: right;position: relative;">
                             Select:
                             <select id="mailcatagory" name="mailcatagory" onchange="changemailkind(this.value)">
                          ...
                          <div class="allmailoperate"><a href="<?=site_url('user/markallread/0')?>">Mark all as Read</a></div>
                          <?php  }?>
 ...
 <?php foreach($mail_list as $_mail): ?>
  ...
  <div class="title">
    <div class="sender">
      ...
      <span class="from"><a href="<?=site_url('user/readmail/'.$_mail['id'].'/sender_id')?>" style="color:<?php echo $_mail['read']==0?'#333':'#ddd'; ?>" style="color:#ddd"><?php echo $_mail['author_name']; ?></a></span>
    </div>
    <div class="sender">
       <span class="time">  
         <a rel="direct" title="Move to Junk Mail?" class="operate_link" href="<?=site_url('user/movemail/'.$_mail['id'].'/1')?>">Move to Junk Mail</a>
         <a title="Are you sure to delete this mail?" class="operate_link" href="<?=site_url('user/deletemail/'.$_mail['id'].'/0')?>" >Delete</a>
         <input type="checkbox" name="choosemail[]" id="mailcheckbox" value="<?php echo $_mail['id']; ?>">
       </span>
...
<div class='paragraphstyle' style='font-weight: bold;font-size:16px;padding:10px;margin-bottom:20px'><?php echo $this->pagination->create_links();?></div>
                          ...
                          <div class="mailoperate"> 
                             ...
                             <input type="submit" name="movechosedmail" class="btn" value="Move to Junk Mail">
                             <input type="submit" name="markchosedmail" class="btn" value="Mark as Read">
                             <!--<a href="<?=site_url('user/markallread/0')?>" style="margin-left:22px;">Mark all as Read</a>-->
                          </div>
                      </div>
                   </div>
                   </form>
