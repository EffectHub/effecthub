$(document).ready(function () {
	...
	$('.catagorypanel' +', ' + '#catagory').hover(function(r){
     ...
     $('.seemore').click(function(){  
     	 ...
     	 $.ajax({  
            type:"GET" 
           ,url:"http://localhost/game/index.php/item/seemore/"+itemID  
           ,data:{id:clickNum}                                
   ...
   $('#watchOrunwatch').click(function(){  
   	 ...
   	 if($(this).html() == "Watch"){
   	 	 $.ajax({  
             type:"GET" 
             ,url:"http://localhost/game/index.php/item/watch/"+itemID  
             ,data:{id:1}                                
             ,contentType:'text/html;charset=utf-8'//编码格式   
             ,success:function(data){  
   		...
   		$.ajax({  
             type:"GET" 
             ,url:"http://localhost/game/index.php/item/unwatch/"+itemID  
             ,data:{id:1}                                
             ,contentType:'text/html;charset=utf-8'//编码格式   
             ,success:function(data){  
               $('#watchOrunwatch').html(data);      
   ...
   $('#likeOrdislike').click(function(){  
     	 ...
     	 if($(this).html() == "Like It!"){
     	 	 $.ajax({  
               type:"GET" 
               ,url:"http://localhost/game/index.php/item/like/"+itemID  
               ,data:{id:1}                                
               ,contentType:'text/html;charset=utf-8'//编码格式   
               ,success:function(data){  
     		...
     		$.ajax({  
               type:"GET" 
               ,url:"http://localhost/game/index.php/item/unlike/"+itemID  
               ,data:{id:1}                                
               ,contentType:'text/html;charset=utf-8'//编码格式   
               ,success:function(data){  
                 $('#likeOrdislike').html(data);      
   ...
   $('.seemore-particle').click(function(){  
   	 ...
   	 $.ajax({  
          type:"GET" 
         ,url:"http://localhost/game/index.php/user/seemore/"+userID  
         ,data:{id:clickNum1}                                
         ,contentType:'text/html;charset=utf-8'//编码格式  
         ,beforeSend:function(data){  
   ...
   $('#postcomment').click(function(){ 
	   ...
	   $.ajax({  
           ...
           ,data:{item_id:itemID,content:commentcontent,parent_id:parentid}                              
           ,contentType:'text/html;charset=utf-8'//编码格式   
           ,success:function(data){  
             $('.commentregion').prepend(data); 
             $('#contentarea').attr("value",'');
           }//请求成功后  
           ,error:function(data){  
              $('.commentregion').prepend('failed to publish your comment.')   
           }//请求错误  
        }); 
   });
	   ...
	   if(url=="http://localhost/game/" || url.indexOf("home") != -1)
	   {
	     ...
	     // 然后判断窗口的滚动条是否接近页面底部，这里的20可以自定义
	      if( $(document).scrollTop() + $(window).height() > $(document).height() - 20 ){
		    ...
		    $.ajax({  
		          type:"GET" 
		         ,url:"http://localhost/game/index.php/home/seemore"                                
		         ,data:{id:scrollNum}
		         ,contentType:'text/html;charset=utf-8'//编码格式  
		         ,beforeSend:function(data){  
